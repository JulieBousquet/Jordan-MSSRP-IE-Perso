...
*iefolder*1*FolderGlobals*Randomization*****************************************
*iefolder will not work properly if the line above is edited
   ...
   *Encrypted round sub-folder globals
   ...
   global rand_dtRaw             "$rand_encrypt/Raw Identified Data" 
   global rand_doImp             "$rand_encrypt/Dofiles Import" 
   global rand_HFC               "$rand_encrypt/High Frequency Checks" 

   *DataSets sub-folder globals
   global rand_dt                "$rand/DataSets" 
   global rand_dtDeID            "$rand_dt/Deidentified" 
   global rand_dtInt             "$rand_dt/Intermediate" 
   global rand_dtFin             "$rand_dt/Final" 

   *Dofile sub-folder globals
   global rand_do                "$rand/Dofiles" 
   global rand_doCln             "$rand_do/Cleaning" 
   global rand_doCon             "$rand_do/Construct" 
   global rand_doAnl             "$rand_do/Analysis" 

   *Output sub-folder globals
   global rand_out               "$rand/Output" 
   global rand_outRaw            "$rand_out/Raw" 
   global rand_outFin            "$rand_out/Final" 

   *Questionnaire sub-folder globals
   global rand_prld              "$rand_quest/PreloadData" 
   global rand_doc               "$rand_quest/Questionnaire Documentation" 
...
*iefolder will not work properly if the line above is edited
   ...
   *            Randomization should be replicated, including output of 
   *            tables, graphs, etc.
